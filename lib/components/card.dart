...
class Card extends PositionComponent {
  ...
  // these properties as static because they will all be the same across all
  // 52 card objects, so we might as well save some resources by having
  // them initialized only once.
  ...
  // The render() method of a PositionComponent operates in a local coordinate system,
  // which means we don’t need to worry about where the card is located on the
  // screen. This local coordinate system has the origin at the top-left corner of the
  // component, and extends to the right by width and down by height pixels.
  //
  // There is a lot of artistic freedom in how to draw the back of a card, but my
  // implementation contains a solid background, a border, a flame logo in the middle,
  // and another decorative border
  ...
  // Calling these sprites redJack, redQueen, and redKing. This is because,
  // after some trial, the images don’t look very well on black-suit cards. So
  // do is to take these images and tint them with a blueish hue. Tinting of
  // a sprite can be achieved by using a paint with colorFilter set to the
  // specified color and the srcATop blending mode.
  ...
  void _renderFront(Canvas canvas) {
    // draw the background and the card border
    ...
    // Draw the rank and the suit symbols in the corners of the card.
    ...
    // Create a big switch statement on the card’s rank, and draw pips
    // accordingly. The code below may seem long, but it is actually quite
    // repetitive and consists only of drawing various sprites in different
    // places on the card’s face
    switch (rank.value) {
      case 1:
  ...
  //  This method will draw the provided sprite on the canvas at the
  //  specified place (the location is relative to the dimensions of the
  //  card). The sprite can be optionally scaled. In addition, if flag
  //  rotate=true is passed, the sprite will be drawn as if it was rotated
  //  180º around the center of the card.
  void _drawSprite(
    Canvas canvas,
    Sprite sprite,
    double relativeX,
